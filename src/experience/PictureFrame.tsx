...
const PICTURE_FRAME_OBJECT_DATA = {
  id: "picture-frame",
  title: "Picture Frame",
  content: "about",
};
...
interface PictureFrameProps {
  ...
  // Maximum distance to interact with the picture frame
...
export default function PictureFrame({
  ...
  useFrame(() => {
    ...
    // Only raycast against the frame model, not other children like sprites
    const intersects = raycaster.intersectObjects(
      frameRef.current.children,
      true
    );

    // Check if frame is within interaction range
    const framePosition = new THREE.Vector3(...position);
    const distanceToFrame = camera.position.distanceTo(framePosition);

    if (
      intersects.length > 0 &&
      intersects[0].distance < 5 &&
      distanceToFrame <= interactionRange
    ) {
  ...
  useEffect(() => {
    if (isLookingAt) {
      setTargetedObject(PICTURE_FRAME_OBJECT_DATA);
    } else if (targetedObject?.id === PICTURE_FRAME_OBJECT_DATA.id) {
  ...
  useEffect(() => {
    return () => {
      if (targetedObject?.id === PICTURE_FRAME_OBJECT_DATA.id) {
        setTargetedObject(null);
      }
    };
  ...
  // Listen for E key press or click when looking at picture frame
  useEffect(() => {
  ...
  // Lighten materials when looking at the picture frame
  useEffect(() => {
  ...
  return (
    <group ref={groupRef} position={position} rotation={rotation}>
      {/* Picture frame model - separate ref for raycasting */}
      <group ref={frameRef}>
